 ...
 * @RequestMapping(EMPLOYEE) diyerek "api/v1/employee" URL'sine gelen istekleri bu sınıfa yönlendirir.
...
public class EmployeeController {
    ...
    public ResponseEntity<EmployeeCreateResponseDto> createEmployee(@RequestBody EmployeeCreateRequestDto dto){
        return ResponseEntity.ok(employeeService.createEmployee(dto));
    }

    @PatchMapping(UPDATE)
    public ResponseEntity<String> softUpdate(@RequestBody EmployeeUpdateRequestDto dto){
        return ResponseEntity.ok(employeeService.softUpdate(dto));
    }
    ...
    @GetMapping(FIND_ALL)
    public ResponseEntity<List<FindAllEmployeesResponseDto>> findAllEmployees(){
        return ResponseEntity.ok(employeeService.findAllEmployees());
    }

    @GetMapping(FIND_BY_ID)
    public ResponseEntity<FindEmployeeByIdResponseDto> findEmployeeById(@PathVariable Long id) {
        return ResponseEntity.ok(employeeService.findEmployeeById(id));
    }

    @GetMapping(FIND_BY_AUTH_ID)
    public ResponseEntity<FindEmployeeByIdResponseDto> findEmployeeByAuthId(@PathVariable Long authId) { //Path verirken constant tarafına da {authId} olarak vermezsek hata alıyoruz.
        return ResponseEntity.ok(employeeService.findEmployeeByAuthId(authId));
    }
    ...
    public ResponseEntity<List<FindAllEmployeesResponseDto>> findEmployeesByCompanyName(String companyName) {
        return ResponseEntity.ok(employeeService.findEmployeesByCompanyName(companyName));
    }
