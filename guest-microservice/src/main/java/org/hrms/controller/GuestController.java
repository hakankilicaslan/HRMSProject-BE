 ...
 * @RequestMapping(GUEST) diyerek "api/v1/guest" URL'sine gelen istekleri bu sınıfa yönlendirir.
...
public class GuestController {
     ...
     */
    @PatchMapping(UPDATE)
    public ResponseEntity<String> softUpdate(@RequestBody GuestUpdateRequestDto dto){
        return ResponseEntity.ok(guestService.softUpdate(dto));
    }
    ...
    public ResponseEntity<String> softDelete(@PathVariable Long id){
        return ResponseEntity.ok(guestService.softDelete(id));
    }

    @GetMapping(FIND_ALL)
    public ResponseEntity<List<FindAllGuestsResponseDto>> findAllGuests(){
        return ResponseEntity.ok(guestService.findAllGuests());
    }

    @GetMapping(FIND_BY_ID)
    public ResponseEntity<FindGuestByIdResponseDto> findGuestById(@PathVariable Long id) {
        return ResponseEntity.ok(guestService.findGuestById(id));
    }

    @GetMapping(FIND_BY_AUTH_ID)
    public ResponseEntity<FindGuestByIdResponseDto> findGuestByAuthId(@PathVariable Long authId) { //Path verirken constant tarafına da {authId} olarak vermezsek hata alıyoruz.
        return ResponseEntity.ok(guestService.findGuestByAuthId(authId));
    }
